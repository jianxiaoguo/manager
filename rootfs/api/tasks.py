...
def generate_bill(owner_id, bill_date):
    ...
    try:
        ...
        with cache.lock("generate_bill:%s:%s" % (owner_id, period), timeout=600):
...
def generate_invoice(owner_id, invoice_date):
    ...
    try:
        ...
        with cache.lock("generate_invoice:%s:%s" % (owner_id, period), timeout=600):
...
def paying_by_invoice(invoice, arrears_handle=block_user.delay):
    task_id = uuid.uuid4().hex
    signals.request_started.send(sender=task_id)
    with cache.lock("paying_by_invoice:%s" % invoice.owner_id, timeout=600):
        try:
...
def supplementary_payment(owner_id):
    with cache.lock("supplementary_payment:%s" % owner_id, timeout=600):
