...
router.beforeEach((to, from, next) => {
  const currentCluster = router.store.getters.currentCluster
  const currentApp = router.store.getters.currentApp

  if (!currentCluster && to.path !== '/apps') {
    return next('/apps')
  }

  if (currentCluster && !currentApp && /^\/apps\/[a-z][a-z0-9-]*[a-z0-9]$/.test(to.path)) {
    return next('/apps')
  }
