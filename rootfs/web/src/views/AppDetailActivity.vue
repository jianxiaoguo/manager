            ...
            <div class="hk-hide-bb-last-row list-group limit-width ember-view">
                <div class="flex bb pv3 b--light-silver activity-item ember-view" v-for="(activity, index) in activities">
                    ...
                    <div class="flex flex-column lh-copy">
                        ...
                        <div class="f5 gray">
                            <span class="b">ptypes: </span>
                            <span>{{activity.deployed_ptypes.toString()}}</span>
                        </div>
                        <div class="f5 gray">
                            <span :title="activity.createdTime" class="timeago ember-view">{{activity.created_time}}</span>
                            ·
                            {{activity.version}}
                            <span class="confirmable-action ember-view" v-if="index !=0 && index != activities.length-1">·
                                <a @click="openRollBack(activity.version)" class="hk-link rollback">
                                    Roll back to here
                                </a>
                            </span>
                        </div>
